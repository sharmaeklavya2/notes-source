...
$N$ has independent and stationary increments and $N(t) \sim \Poisson(\lambda t)$.
\end{definition}

\begin{theorem}[Alternative definition of Poisson process]
A counting process $N$ is a Poisson process with rate $\lambda$ iff
$N$ has independent and stationary increments and
$\Pr(N(t) = 1) = \lambda t + o(t)$ and $\Pr(N(t) \ge 2) = o(t)$.
\end{theorem}
...
\begin{theorem}
Let $N$ be a counting process. Then $N$ is a Poisson process with rate $\lambda$
...
where $N_i$ has rate $\lambda_i$. Let $N \defeq \sum_{i \in K}N_i$.
Then $N$ is a Poisson process with rate $\sum_{i \in K} \lambda_i$.
\end{theorem}

\begin{theorem}[Decomposition theorem 2]
Let $N$ be a Poisson process with rate $\lambda$. Let $K$ be a finite set (called set of labels).
Suppose the $j\Th$ event receives label $L_j \in K$, where $\Pr(L_j = i) = p_i$
and $\{N, L_1, L_2, \ldots\}$ are independent.
For $i \in K$, let $N_i(t)$ be the number of events having label $i$, i.e,
$N_i(t) = \sum_{j=1}^{N(t)} \boolone(L_j = i)$. Then $N_i$ is a Poisson process with rate $\lambda p_i$.
Furthermore, all $N_i$ are independent and $N_i(t) \mid N(t) \sim \opname{Binom}(N(t), p_i)$.
\end{theorem}

\begin{lemma}
Let $N^{(1)}$ and $N^{(2)}$ be independent Poisson procs with rates $\lambda_1$ and $\lambda_2$. Then
