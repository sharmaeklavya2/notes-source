...
\begin{theorem}
Let $X$ be a THCTMC. Let $T_u \defeq \min_{t \ge 0} (X(t) \neq u)$.
Then $(T_u \mid X(0)=u) \sim \Expo(\lambda)$ for some $\lambda$.
\end{theorem}

\begin{theorem}[Chapman-Kolmogorov DiffEqs]
For a TH CTMC $X$, let $\nu_i$ be the transition rate out of state $i$,
let $p_{i,j}$ be the probability of going from state $i$ to $j$,
and $q_{i,j}$ be the transition rate from state $i$ to $j$.
(Then $\nu_i = \sum_j q_{i,j}$ and $p_{i,j} = q_{i,j}/v_i$.)
Let $P_{i,j}(t) \defeq \Pr(X(t)=j \mid X(0)=i)$. Then
\begin{itemize}
...
In an irreducible ergodic TH CTMC $X$, for every state $j$,
...
Let $X$ be an irreducible ergodic TH CTMC. Let $Y$ be the sequence of states visited by $X$.
